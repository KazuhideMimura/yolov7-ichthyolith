...
def generate_csv(detect_sample_dir, img_sample_dir, site_name, class_names, 
    ...
    sample_name = os.path.basename(detect_sample_dir)
    csv_path = os.path.join(os.path.dirname(detect_sample_dir), 
                            'cropped_images', 
                            os.path.basename(detect_sample_dir), 
                            'detections.csv')
        
    df = pd.DataFrame([])
    row = 0
    
    results_dirs = [p for p in glob(os.path.join(detect_sample_dir, '*')) if os.path.isdir(p)]
    results_dirs.sort()
    ...
    if len(df) >= 1:
        ...
        # crop
        if crop_img:
            df = crop_images(df, detect_sample_dir, img_sample_dir, margin = 30, mag = 3, extention=extention)
...
def crop_images(df, detect_sample_dir, img_sample_dir, margin = 30, mag = 3, 
    ...
    for s in sorted(slide_names):
        ...
        for index, item in dfs.iterrows():
            ...
            img_path = os.path.join(img_sample_dir, s, img_name)
            ...
            if use_default_savedir:
                save_dir = os.path.join(os.path.dirname(detect_sample_dir), 'cropped_images', sample_name, class_name)
